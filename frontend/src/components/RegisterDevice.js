...
class RegisterDevice extends Component {
  ...
  generateEcKeyPair(curve) {
    ...
    console.log(
      `Public key uncompressed: ${keyPair.getPublic().encode("hex")}`
    );
  ...
  getContentForStep(step) {
    ...
    if (step === 0) {
      ...
      const ecMenu = (
        <Menu onClick={(e) => this.onCurveSelect(e)}>
          {curves.map((curve) => (
            <Menu.Item key={curve}>{curve}</Menu.Item>
          ))}
        </Menu>
      );
      return (
        <div>
          <p>
            <strong>Unique device identifier</strong> is a public key or a
            fingerprint of RSA/ECC public key. It can also be an Ethereum
            address (recommended).
          </p>
          <Input
          ...
          <Button.Group size='large'>
            <Button type='primary' onClick={() => this.generateEthWallet()}>
              Generate Ethereum wallet
            </Button>
            <Dropdown overlay={ecMenu}>
              <Button type='primary'>Generate elliptic curve key pair</Button>
            </Dropdown>
          </Button.Group>
          {this.state.showIdentifierInfo ? (
            <div>
              <br />
              <Alert
                message='You will be given private key and device configuration on the last step.'
                type='info'
                showIcon
              />
            </div>
    ...
    if (step === 1) {
      return (
        <div>
          <p>
            <strong>Metadash hash</strong> is Merkle root hash of device
            information or just a hash of any data.
          </p>
          <Input
          ...
          <Divider />
          <p>
            If you already don't have one, you can use inputs below to generate
            SHA-3 (Keccak) hash. With multiple fields, Merkle tree will be used.
          </p>
          <br />
          <Form>
            ...
            <FormItem>
              <Button
                type='primary'
                onClick={() => this.calculateMetadataHash()}
              >
                Generate
              </Button>
            </FormItem>
    ...
    if (step === 2) {
      return (
        <div>
          <p>
            <strong>Firmware hash</strong> is a hash of actual firmware hash.
            Actual firmware hash is not supposed to be stored.
          </p>
          <Input
          ...
          />
          <Divider />
          <p>You can use input to generate SHA-3 (Keccak) hash of any data.</p>
          <br />
          <Input
          ...
          <br />
          <br />
          <Button type='primary' onClick={() => this.calculateFirmwareHash()}>
            Generate
          </Button>
        </div>
      );
          ...
          <Card
            title={
              <div>
                ...
                <a>
                  <Icon type='edit' onClick={() => this.gotoStep(0)} />
                </a>
              </div>
            }
            bordered={false}
          >
            <Meta
              title={
                <div>
                  Metadata hash{" "}
                  {metadataHash.length > 0 ? metadataHash : "empty"}{" "}
                  <a>
                    <Icon type='edit' onClick={() => this.gotoStep(1)} />
                  </a>
                </div>
              }
              description={
                <div>
                  Firmware hash{" "}
                  {firmwareHash.length > 0 ? firmwareHash : "empty"}{" "}
                  <a>
                    <Icon type='edit' onClick={() => this.gotoStep(2)} />
                  </a>
                </div>
              }
            />
    ...
    if (step === 4) {
      return (
        <div style={{ textAlign: "center" }}>
          ...
          <br />
          <br />
          <p>Click below to download device configuration.</p>
          <br />
          <Button type='primary' onClick={() => this.downloadConfiguration()}>
            Download
          </Button>
        </div>
      );
  ...
  async createDevice() {
    ...
    try {
      ...
      if (address !== "") {
        ...
        identifierToSave = setLengthLeft(
          Buffer.from(addressToPad, "hex"),
          32
        ).toString("hex");
      }
      console.log("Before result", addHexPrefix(identifierToSave), instance);
      // const accounts = await web3.request({
      //   method: "eth_requestAccounts",
      // });
      getDefaultAccount().then((value) => console.log(value));
      let result = await instance.createDevice(
        addHexPrefix(identifierToSave),
        addHexPrefix(metadataHash),
        addHexPrefix(firmwareHash),
        { from: "0x9788c5811aDda5FD40CCBF4395a5153319A59176" }
      );
      ...
      openNotificationWithIcon(
        "info",
        "Transaction sent",
        "Once mined, your device will be registered."
      );
      this.setState({
        loading: true,
      });
      ...
      <div>
        <Spin spinning={this.state.loading} className='loading-spin'>
          ...
          <div className='steps-action'>
            ...
            {current === steps.length - 1 && (
              <Button type='primary' onClick={() => this.createDevice()}>
                Register
              </Button>
            )}
            {current > 0 && current !== 4 && (

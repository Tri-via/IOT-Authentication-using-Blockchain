...
const hashMessageHex = (message) =>
  addHexPrefix(hashPersonalMessage(Buffer.from(message)).toString("hex"));

app.use(express.json());

const DeviceManagerArtifact = require("../build/contracts/DeviceManager.json");
let latestNetwork =
  DeviceManagerArtifact.networks[
    Object.keys(DeviceManagerArtifact.networks).reduce((res, curr) =>
      curr > res ? curr : res
    )
  ];
...
const deviceManager = web3.eth
  .contract(DeviceManagerArtifact.abi)
  .at(latestNetwork.address);
...
app.post("/receive", function (req, res) {
  ...
  let validMessage = deviceManager.isValidEthMessage(
    deviceId,
  ...
  let validMetadata = deviceManager.isValidMetadataMember(
    deviceId,
    proof,
    metadataHash
  );
...
app.listen(1337, () =>
  console.log("Platform simulation listening on port 1337")
);
